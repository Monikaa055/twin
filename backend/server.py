...
aws_region = os.getenv("DEFAULT_AWS_REGION") or os.getenv("AWS_REGION") or "ap-south-1"
...
async def health_check():
    ...
    try:
        from context import prompt
        test_prompt = prompt()
        health_status["resources_loaded"] = True
        health_status["prompt_length"] = len(test_prompt)
    except Exception as e:
        health_status["status"] = "degraded"
        health_status["resources_loaded"] = False
        health_status["resources_error"] = str(e)
    
    # Check Bedrock client
    try:
        # Just check if client is initialized
        health_status["bedrock_client_initialized"] = bedrock_client is not None
    except Exception as e:
        health_status["status"] = "degraded"
        health_status["bedrock_client_initialized"] = False
        health_status["bedrock_error"] = str(e)
...
async def chat(request: ChatRequest):
    try:
        ...
        # In production, don't expose full traceback to user
        error_detail = str(e) if os.getenv("ENVIRONMENT") != "prod" else "Internal server error"
